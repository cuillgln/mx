(function() {
	var SafetyMonitoringStation = mxLib.SafetyMonitoringStation = function() {
	 ...
	 * 加载位置
	 */
	SafetyMonitoringStation.prototype.getPosition = function() {
		var self = this;
		$.ajax({
	 ...
	 * 保存传感器位置
	 */
	SafetyMonitoringStation.prototype.savePosition = function() {
		var positionData = [];
	...
	SafetyMonitoringStation.prototype.getStation = function() {
		var self = this;
		var arr = [];
		$.ajax({
	...
	// 设置实时值
	SafetyMonitoringStation.prototype.updateInternal = function() {
		var self = this;
	...
	SafetyMonitoringStation.prototype.setValue = function(stationId, value, alarmFlag) {
	...
	SafetyMonitoringStation.prototype.addMarker = function(opts) {
		...
		if (opts.alarmFlag == 1) {
			strTotalPath = strBasePath + "sms-alarm.jpg";
		}
		// 构造html 无实时值
	...
	SafetyMonitoringStation.prototype.updatePoint = function(stationId, point) {
	...
	SafetyMonitoringStation.prototype.removePoint = function(stationId) {
		var marker = this.markerMap[stationId];
		if (!$.isEmptyObject(marker) && marker instanceof mxLib.RichMarker) {
			map.removeOverlay(marker);
	...
	SafetyMonitoringStation.prototype.getPoint = function(stationId) {
		if ($.isEmptyObject(this.positionMap)) {
			return null;
		}
		if (this.positionMap.hasOwnProperty(stationId)) {
			return this.positionMap[stationId];
		}
	}
	
	SafetyMonitoringStation.prototype.addEvent = function(marker) {
		if ($.isEmptyObject(marker)) {
			return;
		}
		...
		marker.addEventListener("click", function() {
			...
			// 运用ifame框架
			var sensorInfoUrl = contextPath + "/sensorinfo?sensorId=" + stationId + "&deviceType=2";
	...
	// 定位
	SafetyMonitoringStation.prototype.dolly = function(stationId) {
		if (stationId == "")
			return;
	...
	SafetyMonitoringStation.prototype.autoRefresh = function() {
